...
export class CloudService {
   ...
   * 获取用户打卡记录
   */
  async getCheckinRecords(openid: string): Promise<CheckinRecord[]> {
    try {
   ...
   * 删除打卡记录
   */
  async deleteCheckinRecord(recordId: string): Promise<boolean> {
    try {
   ...
   * 清空用户所有打卡记录
   */
  async clearAllCheckinRecords(openid: string): Promise<boolean> {
    try {
      const { data } = await this.db.collection(COLLECTIONS.CHECKIN_RECORDS)
        .where({
          _openid: openid
   ...
   * 检查今天是否已打卡
   */
  async isTodayChecked(openid: string): Promise<boolean> {
    try {
   ...
   * 获取用户统计信息
   */
  async getUserStats(openid: string): Promise<{
    totalCount: number;
   ...
   * 计算连续打卡天数
   */
  private async calculateContinuousDays(openid: string): Promise<number> {
    try {
      const { data } = await this.db.collection(COLLECTIONS.CHECKIN_RECORDS)
        .where({
          _openid: openid
      ...
      for (let i = 0; i < 365; i++) {
        ...
        const hasRecord = data.some((record: CheckinRecord) => 
   ...
   * 保存用户信息
   */
  async saveUserProfile(openid: string, userInfo: UserInfo): Promise<boolean> {
    try {
   ...
   * 获取用户信息
   */
  async getUserProfile(openid: string): Promise<UserInfo | null> {
    try {
